...
//   └── boot/efi.img            the FAT16 ESP; El Torito UEFI boot record
//                               points here, so firmware reading the CD
//                               uses this file as the boot volume.
//
// All four files are passed in on the command line; we stage them in a
// temp workspace and hand it to github.com/diskfs/go-diskfs to render
// the ISO.
//
// Usage:
//
//	mkiso out.iso esp.img BOOTX64.EFI BOOTAA64.EFI
package main
...
func main() {
	if len(os.Args) != 5 {
		fmt.Fprintln(os.Stderr, "usage: mkiso <out.iso> <esp.img> <BOOTX64.EFI> <BOOTAA64.EFI>")
		os.Exit(2)
	}
	outPath := os.Args[1]
	espPath := os.Args[2]
	...
	// BOOTX64.EFI / BOOTAA64.EFI copies under /EFI/BOOT/ are not
	// strictly required for boot, but they make the ISO usable as a
	// loopback-mounted source on a running OS.
	...
	stage(x64Path, "EFI/BOOT/BOOTX64.EFI")
	stage(aa64Path, "EFI/BOOT/BOOTAA64.EFI")
